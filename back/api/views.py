...
from .models import Project, Invoice, Client, Dev
...
class StatsView(RetrieveAPIView):
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        if dev_id is not None:
            ...
            invoices = Invoice.objects.filter(project__in=projects)
            clients = Client.objects.filter(project__in=projects)
            globalEarnings = Dev.objects.filter(id=dev_id).values('globalEarnings')
            projectCount = projects.count()
            clientCount = clients.count()
            invoice_value = invoices.aggregate(Sum('invoice_value'))
            stat = {
                'projectCount': projectCount,
                'clientCount': clientCount,
                'invoice_value': invoice_value['invoice_value__sum'],
                'globalEarnings': globalEarnings[0]['globalEarnings'],
            }
